...
#
# DESCRIPTION
# Packager for sphinxclient library.
#
# AUTHORS
...
# what library version conflicts
CONFLICTS_LIBRARY_VERSION=0
...
while [ "$#" != "0" ]; do
    case "$1" in
        --help)
            ...
            echo "    libsphinxclient[-dev].control file."
            echo ""
            echo "    You can also create libsphinxclient[-dev].postinst, libsphinxclient[-dev].preinst,"
            echo "    libsphinxclient[-dev].conffiles, libsphinxclient[-dev].prerm and libsphinxclient[-dev].postrm files"
...
function make_dirs {
    ...
    if [ "${dash}" = "" ]; then
        ...
        CONFLICTS=${PROJECT_NAME}${CONFLICTS_LIBRARY_VERSION}
    else
        # libsphinxclient0-dev
        name=$(echo ${PROJECT_NAME} | cut -f1 -d'-')
        suff=$(echo ${PROJECT_NAME} | cut -f2- -d'-')
        PACKAGE_NAME=${name}${LIBRARY_VERSION}-${suff}
        
        CONFLICTS=${name}${CONFLICTS_LIBRARY_VERSION}-${suff}
    fi
...
if [ "${MODE}" = "binary" ]; then
    ...
    # build extra depend
    SH_DEPEND=$(dpkg-shlibdeps -O ${INSTALL_DIR}/usr/lib/libsphinxclient.so | \
    ...
    # Compose extra dependencies: we must depend on libsphinxclient library with
...
# project name
PROJECT_NAME="libsphinxclient"

# Maintainer of this module
MAINTAINER="Jan Kirschner <jan.kirschner@firma.seznam.cz>"
...
if test -z "${SKIP_INSTALL}"; then
    (
        cd ..
        # install libsphinxclient to the install-dir
...
# Determine library version -- we generate libsphinxclient<LIBRARY_VERSION>
# and libsphinxclient<LIBRARY_VERSION>-dev packages.
export LIBRARY_VERSION=$(sed -n -e 's/current=\(.*\)/\1/p' \
        ${INSTALL_DIR}/usr/lib/libsphinxclient.la) || exit -1
